...
public class InsertQueries {
  ...
  /**
   * Úkol: Vaším úkolem je pøiøadit INSERT pøíkaz do promìnné insertUser, tak aby se vytvoøil nový
   * uživatel dle zadaných parametrù
   * 
   * HINT: V této metodì jsou využity preparedStatements, takže se parametr emailu nastaví až
   ...
   */
  public void insertNewUser(String email, String name, String surname, String tableName) {//variabile tablename
    if (email == null || name == null || surname == null)
      throw new NullPointerException("Email, name and surname must be set.");

    Connection conn = DBConnection.getDBConnection();
    String insertUser = "INSERT INTO "+tableName+" (email,name,surname) " + "VALUES (?,?,?)";
